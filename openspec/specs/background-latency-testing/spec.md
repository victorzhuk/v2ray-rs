...
The system SHALL run a session-local background latency refresh every 10 minutes for enabled nodes in enabled subscriptions while the app is running. The 10-minute interval balances freshness with network overhead for typical VPN usage patterns.
...
On startup, the system SHALL hydrate `last_latency_ms` for all subscription nodes from the persisted `latency_snapshot.json`, so the UI displays known latency values immediately rather than waiting for the first scheduled refresh.
