...
- **WHEN** the app launches and the resolved storage directories do not exist
- **THEN** the system SHALL create config, data, cache, runtime, and state directories with `0o700` permissions

#### Scenario: XDG override
- **WHEN** any of `XDG_CONFIG_HOME`, `XDG_DATA_HOME`, `XDG_CACHE_HOME`, `XDG_RUNTIME_DIR`, or `XDG_STATE_HOME` is set
- **THEN** the system SHALL place that directory under the overridden root, joined with the profile qualifier

#### Scenario: Profile isolates storage
- **WHEN** the same user launches the binary with `--profile production` and later with `--profile development`
...
- **WHEN** the user launches with `--data-dir /tmp/scratch/data`
...
#### Scenario: First launch writes the stamp
