...
- Redaction as post-hoc `serde_json::Value` walk masking values at `{id, uuid, password, short_id}` (union across backends — masking per-backend key subsets risks leaking `uuid` while on xray, etc.). Alternative — threading a redact flag through three generators — rejected: display concern doesn't belong in generation.
