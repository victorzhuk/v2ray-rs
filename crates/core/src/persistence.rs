...
use crate::models::{
    AppSettings, ConnectionMetadata, ManualNode, Preset, RoutingRuleSet, Subscription,
};
...
pub fn save_manual_nodes(paths: &AppPaths, nodes: &[ManualNode]) -> Result<(), PersistenceError> {
    ...
    atomic_write(&paths.custom_nodes_path(), json.as_bytes())
}
...
pub fn load_manual_nodes_or_default(paths: &AppPaths) -> Vec<ManualNode> {
    match load_manual_nodes(paths) {
        ...
        Err(PersistenceError::Json(_)) => {
            eprintln!("Warning: corrupt custom_nodes.json file. Using empty manual nodes list.");
            Vec::new()
        }
        Err(e) => {
            eprintln!("Warning: failed to load manual nodes: {e}. Using empty list.");
            Vec::new()
        }
    }
}
...
mod tests {
    ...
    fn test_manual_nodes_save_load_roundtrip() {
        ...
        use crate::models::{ManualNode, ProxyNode, VlessConfig};
        ...
        save_manual_nodes(&paths, &[manual.clone()]).unwrap();
    ...
    fn test_manual_nodes_roundtrip_with_real_data() {
        let (_tmp, paths) = test_paths();
        use crate::models::{ManualNode, ProxyNode, ShadowsocksConfig};
        ...
        save_manual_nodes(&paths, &[manual1.clone()]).unwrap();
